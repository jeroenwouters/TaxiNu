...
var AdminPanel = new(Backbone.Router.extend({
	...
	initialize: function() {
		...
		this.rittenListView = new RittenListView({
			collection: this.rittenList
		});
	},
	start: function() {
...
function gotPosition(position) {
	...
	if (you) you.setPosition(pos);
		...
		google.maps.event.addListener(you, 'click', function() {
			location = 'mailto:?subject=GPS%20Track&body=' + encodeURIComponent(log + ' ]\n}\n');
		});
	}
	...
	map.fitBounds(markerBounds);
	channel2.trigger('client-taxi_' + current_taxi_id, {
		lat: position.coords.latitude,
		lang: position.coords.longitude
	});
	channel3.trigger('client-taxis', {
		lat: position.coords.latitude,
		lang: position.coords.longitude,
		taxi: current_taxi_id
	});
